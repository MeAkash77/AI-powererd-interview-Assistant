...
class RAGOrchestrator {
  ...
  async processTranscript(transcript: string, background?: string): Promise<RAGResponse> {
    // ✅ ALWAYS USE RAG - Force RAG pipeline regardless of knowledge check
    console.log("⚡ Forcing RAG pipeline...");
    ...
    try {
      ...
      try {
        ...
        console.warn('⚠️ Question extraction failed, trying local extraction:', (error as Error).message || 'Unknown error');
      ...
      // ✅ FIXED: Handle empty query by using full transcript
      if (!searchQuery || searchQuery.trim() === "") {
        console.log("⚠️ Empty query detected, using full transcript as query");
        searchQuery = generateSearchQuery(transcript);
        console.log('🔄 Using generated search query:', searchQuery);
      }

      // If we still don't have a meaningful query, use transcript as fallback
      if (!searchQuery || searchQuery.trim() === "") {
        console.log("⚠️ Still no query, using first 200 chars of transcript");
        searchQuery = transcript.substring(0, 200);
      }

      console.log(`🔍 Final search query: "${searchQuery.substring(0, 100)}..."`);
  ...
  async processMultimodalTranscript(
    ...
    // ✅ ALWAYS USE RAG - Force RAG pipeline for multimodal as well
    console.log("⚡ Forcing RAG pipeline for multimodal...");
  ...
  private async processImageForContext(imageFile: File): Promise<string> {
    try {
      ...
      // ✅ FIXED: Use correct model name "gemini-1.0-pro"
      const model = this.gemini.getGenerativeModel({ model: 'gemini-1.0-pro' });
